<?php 
function getURI($parametre = false, $param_to_delete = array()){
    //On convertit le second param?re en array si ce n'en est pas une
    ...
    foreach($_GET as $cle => $valeur){ //On parcourt toutes les variables en GET
        if(!in_array($cle, $param_to_delete)){ //On regarde si la variable fait partie de celles ?supprimer
            //Sinon, on l'ajoute ?l'adresse
            $adresse .= ($adresse ? '&' : '?').$cle.($valeur ? '='.$valeur : '');
        }
    }
    if($parametre){
        //Si le premier param?re de la fonction est true, on ajoute le n?essaire pour pouvoir ajouter une variable apr? le retour de la fonction
        $adresse .= ($adresse ? '&' : '?');
    }
    //On ajoute le chemin du fichier sans les param?res GET
...
function GetMontantMembreSessionsCommission($theCommission_id, $thePersonne_id, $theFonction_id, $theMonth_1, $theMonth_2, $theYears)  
{
		...
		$password_MyFileConnect = "";
		$MyFileConnect = mysql_pconnect($hostname_MyFileConnect, $username_MyFileConnect, $password_MyFileConnect) or trigger_error(mysql_error(),E_USER_ERROR);
		$query_rsMembres = sprintf("SELECT commission_id, fonctions_fonction_id, (sum(nombre_jour) * montant) as total
		FROM sessions, membres, commissions, personnes
		WHERE sessions.membres_commissions_commission_id = membres.commissions_commission_id 
		AND sessions.membres_personnes_personne_id = membres.personnes_personne_id
		AND commissions.commission_id = membres.commissions_commission_id
		AND personnes.personne_id = membres.personnes_personne_id
		AND personnes.display = '1'
		AND (commissions.commission_id = %s OR commissions.commission_parent = %s)
		AND annee = %s
    	AND personnes.personne_id = %s
		AND mois BETWEEN %s AND %s
		AND etat_validation = 2 
		AND etat_appur = 0
    	GROUP BY fonctions_fonction_id, personnes.personne_id", GetSQLValueString($theCommission_id, "int"), GetSQLValueString($theCommission_id, "int"), GetSQLValueString($theYears, "text"),GetSQLValueString($thePersonne_id, "text"),GetSQLValueString($theMonth_1, "text"),GetSQLValueString($theMonth_2, "text"));
		$rsMembres = mysql_query($query_rsMembres, $MyFileConnect) or die(mysql_error());
		$row_rsMembres = mysql_fetch_assoc($rsMembres);
		$totalRows_rsMembres = mysql_num_rows($rsMembres);
		

//$nombre_jours = MinmapDB::getInstance()->SelectMontantByFonctionTYpe($theCommission_id, $theCommission_id,$theYears, $thePersonne_id, $theMonth_1, $theMonth_2);
$montant=0; 
do { 
  switch ($row_rsMembres['fonctions_fonction_id']) {
	case 1:
	...
	break;
	case 40:
	  $somme4 = $row_rsMembres['total'];
	break;
    case 141:
	  $somme5 = $row_rsMembres['total'];
	break;

  }
  $montant = $somme1 + $somme2 + $somme3 + $somme4 + + $somme5;
 } while ($row_rsMembres = mysql_fetch_assoc($rsMembres));
   return $montant;
}
