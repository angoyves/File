...
$query_recordSetBasic2 = "SELECT commission_id, commission_lib, commission_sigle
FROM commissions
WHERE display_agescom = 1";
...
if (isset($txtSearch) && $txtSearch != "-1"){
	...
	$query_recordSet = sprintf(" %s  
	AND (personne_nom LIKE %s OR personne_prenom LIKE %s OR fonction_lib LIKE %s)  
	ORDER BY personne_nom, date_constation ASC", $query_recordSetBasic, 
	GetSQLValueString("%" . $txtSearch . "%", "text"), GetSQLValueString("%" . $txtSearch . "%", "text"), GetSQLValueString("%" . $txtSearch . "%", "text"));
	
	$query_recordSet2 = sprintf(" %s  
	AND (commission_lib LIKE %s OR commission_sigle LIKE %s)  
	ORDER BY commission_lib ASC", $query_recordSetBasic2, 
...
$recordSet2 = mysql_query($query_limit_recordSet2, $MyFileConnect) or die(mysql_error());
...
if (isset($_GET['totalRows_recordSet'])) {
  $totalRows_recordSet2 = $_GET['totalRows_recordSet'];
} else {
...
<script type="text/javaScript" language="javascript">
	...
	function fn_moveCommissionDetails(userId){
		...
		var url = "commissions/CommissionDetail.php?bizRegNo="+userId;
		//form.userId.value = userId;
		form.bizRegNo.value = userId;
		//form.target = "_top";
...
<body onLoad="fn_onLoad()">
    	<?php if (isset($txtSearch) && $txtSearch != "") { ?>
              ...
              Enregistrement trouvé pour votre recherche &quot; <strong> <?php echo $txtSearch; ?></strong><br>
        <?php } ?>
<div id="frameDiv">
<!-- <div class="content"> -->
	<div class="tableTy1">
	  <form name="frm_pubPayment_list" modelAttribute="RcvSVO" method="post">
        ...
        <h2 class="bullet1">MEMBRE DE LA COMMISSION</h2>
          <table class="data rowEven">
            <caption>
              <l:mapping programId="PubPayment" objId="DBF"></l:mapping>
              COMISSION DE PASSATION DES MARCHES PUBLICS
            </caption>
            <colgroup>
              <col style="width: 6%">
              <col style="width: 39%">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td nowrap class="tL"><img src="images/icons/young-user-icon.jpg" width="18" height="16">&nbsp;<?php //echo strtoupper($row_recordSet['personne_nom'].' '.$row_recordSet['personne_prenom']); ?>&nbsp;
                  
                  
                  <a href="#" onClick="fn_moveCommissionDetails('<?php echo $row_recordSet['commission_id']; ?>');">
                    <?php if (isset($txtSearch) && $txtSearch != "") {
                               echo str_ireplace($txtSearch,'<span style="background-color:yellow">'. strtoupper($txtSearch) .'</span>',strtoupper(htmlentities($row_recordSet['personne_nom'] . ' '. ucfirst(htmlentities($row_recordSet['personne_prenom'])))));
              } else { echo strtoupper(htmlentities($row_recordSet['personne_nom'] . " " . ucfirst(htmlentities($row_recordSet['personne_prenom']))));} ?> 
                    </a>
                ...
                <td nowrap class="tL"><?php 
                                  ...
                                  ?>
                  &nbsp; </td>
                <td nowrap class="tL"><a href="#" onClick="fn_moveCommissionDetails('<?php echo $row_recordSet['commission_id']; ?>');"><?php //echo strtoupper($row_recordSet['commission_sigle']); ?>
                  
                  
                  <?php if (isset($txtSearch) && $txtSearch != "") {
                                        echo str_ireplace($txtSearch,'<span style="background-color:yellow">'. strtolower($txtSearch) 
                                        .'</span>',strtoupper(htmlentities($row_recordSet['commission_sigle'] )));
                                    } else { echo strtoupper(htmlentities($row_recordSet['commission_sigle']));} ?>
        ...
        <?php } // Show if recordset not empty ?>       
             <br>  <?php echo $totalRows_RecordSet; //echo $query_recordSet2; ?>   
        <?php if (isset($totalRows_recordSet2) &&  $totalRows_recordSet2 <> 0) { // Show if recordset empty ?>    
        
        <h2 class="bullet1">COMMISSION DE PESSATION DES MARCHES PUBLICS</h2>
        <table class="data rowEven">
                        <caption>
                          <l:mapping programId="PubPayment" objId="DBF"></l:mapping>
                </caption>
                        ...
                        <thead>
                          <tr>
                            <th scope="col">N°</th>
                            <th scope="col">Libelle Commission</th>
                        ...
                        <tbody>
                          ...
                          <tr>
                           ...
                           <?php if (isset($txtSearch) && $txtSearch != "") {
                               echo str_ireplace($txtSearch,'<span style="background-color:yellow">'. strtoupper($txtSearch) .'</span>',strtoupper(htmlentities($row_recordSet2['commission_lib'] )));
              } else { echo strtoupper(htmlentities($row_recordSet2['commission_lib']));} ?>                    &nbsp; </td>
                            <td nowrap class="tL"><a href="#" onClick="fn_moveCommissionDetails('<?php echo $row_recordSet2['commission_id']; ?>');">
                            <?php if (isset($txtSearch) && $txtSearch != "") {
                                        echo str_ireplace($txtSearch,'<span style="background-color:yellow">'. strtoupper($txtSearch) 
                                        .'</span>',strtoupper(htmlentities($row_recordSet2['commission_sigle'] )));
                                    } else { echo strtoupper(htmlentities($row_recordSet2['commission_sigle']));} ?>
