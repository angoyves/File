...
// TYPE LOCALITE 
	...
	public function get_value_by_id($table, $champ_lib, $champ_id, $value_id) {
        $domain = $this->query("SELECT ".$champ_lib." FROM ".$table." WHERE ".$champ_id." = " 
							   . $value_id );
	...
	public function get_id_by_lib($table, $champ_id, $champ_lib, $value_lib) {
        $domain = $this->query("SELECT ".$champ_id." FROM ".$table." WHERE ".$champ_lib." LIKE '%"
		. $value_lib . "%'");
	...
	public function get_max_id($table, $champ_id) {
        $domain = $this->query("SELECT MAX(".$champ_id.") FROM ".$table);
	...
	public function get_value_by_2champ($table, $value, $champ1, $champ2, $value1, $value2) {
        $domain = $this->query("SELECT ".$value." FROM ".$table." WHERE ".$champ1." = " 
							   .$value1." AND ".$champ2." = ".$value2 );
...
// PERSONNES
	...
	public function get_person_id_by_name_and_prenom($firstname, $lastname) {
        $firstname = $this->real_escape_string($firstname);
		$num_cpte = $this->real_escape_string($lastname);
        $domain = $this->query("SELECT personnes.personne_id FROM personnes, rib WHERE personnes.personne_id = rib.personne_id 
							   AND personne_nom = '". $firstname . "' AND numero_compte = '". $lastname . "'");
...
// FIN COMMISSIONS
    ...
    public function get_count_id_by_values($table, $chmp_id, $chmp1, $chmp2, $chmp3, $value1, $value2, $value3) {
        $domain = $this->query("SELECT count(".$chmp1.") as total FROM ".$table." WHERE ".$chmp1." = " 
							   . $value1 . " AND ".$chmp2." = " 
							   . $value2 . " AND ".$chmp3." = " 
							   . $value3 );
    ...
    public function get_value_id_by_value($value_id, $table, $search_value, $value) {
	 ...
	 //public function get_value_id_by_values($value_id, $table, $search_value1, $search_value2, $value1, $value2) {
	 public function get_value_id_by_values($value1, $value2) {
        //$name = $this->real_escape_string($value);
		//$value_id = $this->real_escape_string($value_id); 
		//$table = $this->real_escape_string($table); 
		//$search_value1 = $this->real_escape_string($search_value1); 
		//$search_value2 = $this->real_escape_string($value); 
		//$value1 = $this->real_escape_string($value1); 
		//$value2 = $this->real_escape_string($value2);
        //$domain = $this->query("SELECT '" . $value_id . "' FROM '" . $table . "' WHERE '" . $search_value1 . "' = "
                       // . $value1 . " AND '" . $search_value2 . "' = "
                       // . $value2 );
        $domain = $this->query("SELECT personne_id FROM personnes WHERE personne_nom = '"
                        . $value1 . "' AND personne_prenom = '"
                        . $value2 ."'" );					  
...
//------------------ personnes--------------
	...
	function verify_user_id($personne_id) {
        $domain = $this->query("SELECT 1 FROM users WHERE personne_id = ". $personne_id );
        if ($domain->num_rows > 0){
            $row = $domain->fetch_row();
            return $row[0];
        } else
	...
	function verify_members_id($personne_id) {
        $domain = $this->query("SELECT 1 FROM membres WHERE fonctions_fonction_id = 142 AND personnes_personne_id = ". $personne_id );
        if ($domain->num_rows > 0){
            $row = $domain->fetch_row();
            return $row[0];
        } else
