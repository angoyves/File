...
// FONCTION DE CONTROLE DES MOIS DANS LA TABLE APPUREMENT
    ...
    public function get_montant_appurement_by_rib($rib, $commission_id) {
		$rib = $this->real_escape_string($rib);
...
//SESSIONS
    public function validate_sessions($champ, $value, $duedate, $mois, $annee, $commission_id, $user_id) { 
		$value = $this->real_escape_string($value);
		$mois = $this->real_escape_string($mois); 
		$annee = $this->real_escape_string($annee); 
		$champ = $this->real_escape_string($champ);
		$this->query("update sessions join commissions on sessions.membres_commissions_commission_id=commissions.commission_id
					set etat_validation = '" . $value . "', userValidate = '" . $user_id . "'
					where commission_id = 527 or commission_parent = '" . $commission_id . "'
					and mois = '" . $mois . "'
					and annee ='" . $annee . "'");
    }
...
// FIN COMMISSIONS
    ...
    public function get_nombre_jour_by_values($commission_id, $personne_id, $month, $year) {
        ...
        $domain = $this->query("SELECT count(sessions.Nombre_jour) FROM sessions, commissions 
								...
								. " AND etat_validation = 2 AND sessions.annee = '" . $year 
								. "'");
