...
//**************************************
	...
	$LibMan_Connect = mysql_pconnect($host,$usr,$pass) or die("Erreur de connexion à la base de donn&eacute;es...<br>".mysql_error());
	mysql_select_db($db) or die ("Erreur lors de la selection de la base de donn&eacute;es ".mysql_error());
}
//***** Fin chaîne de connexion *****

//Lorskon a un enregistrement null, on retourne une valeur par défaut prédéfinie
...
//---> Retournera soit une chaine correspondant à notre rs, soit la valeur "Masculin"
//****************************************************************************/
...
****************************************************************************/
	...
	function show_last_where($tbl, $field, $where_field, $where_value){
		...
		$result = mysql_query($query, $LibMan_Connect)  or die("Erreur de selection <br />".mysql_error());
...
function chk_entry($tbl, $field, $entry)  //$tbl = latable a indexer, $field = le champ concerné, $entry = la valeur à tester prise ds le formulaire
	...
	$result = mysql_query($query, $LibMan_Connect) or die ("Erreur d'extraction des données dans la base ".mysql_error());
	$row = mysql_fetch_row($result);
...
function chk_entry_1($tbl, $field, $entry, $fieldtest, $entrytest)  //$tbl = latable a indexer, $field = le champ concerné, $val est la valeur fixe à comparer, $entry = la valeur à tester prise ds le formulaire
	...
	$result = mysql_query($query, $LibMan_Connect) or die ("Erreur d'extraction des données de la base ".mysql_error());
	$row = mysql_fetch_row($result);
	if($total = $row[0])  //Mysql à généré au moins 1 resultat 
	return true;
	}
...
function adduser($login,$pass,$tbl)
    {
	global $LibMan_Connect;
	$result = mysql_query("INSERT INTO $tbl (login, password) VALUES ($login, $password)", $LibMan_Connect);
	return mysql_insert_id($LibMan_Connect);
    }
//*********** Fin d'ajout d'utilisateur ****************
	...
	function set_connected($table, $field, $login_field, $user_entry, $val){
		...
		$result = mysql_query($query,$LibMan_Connect) or die("Erreur de mise à jour de champ <br>".mysql_error());
	}
	...
	function add_entry5($table,$field1,$field2,$field3,$field4,$field5,$entry1,$entry2,$entry3,$entry4,$entry5){
		...
		$result = mysql_query($query,$LibMan_Connect) or die("Erreur d'ajout d'enregistrement:<br>".mysql_error());
		$total = mysql_affected_rows($LibMan_Connect);
		if($total>=1){
...
// *** Supprimer un enregistrement ds un champ $field spécifié de la bdd ***
	function rem_entry($table, $field_where, $entry){
		...
		$result = mysql_query($query, $LibMan_Connect) or die("Erreur de retrait d'une entr&eacute;e dans la bdd <br>".mysql_error());
	}
...
// * Charger les valeurs d'un champ $field ds un combobox***
	...
	/*$table = la table de la bdd
	  ...
	  $val = la valeur à inserer ds les options, ki seront récupérées*/
		...
		$result = mysql_query($query,$LibMan_Connect) or die("Erreur de s&eacute;lection de donn&eacute;es ds la bdd <br>".mysql_error());
		if($total = mysql_num_rows($result)){
			while($row = mysql_fetch_object($result)){
				$val = $row->$field;
...
// * Charger les valeurs d'un champ $field ds un combobox conditioné par une valeur $valwhere du champ $wherefield***
	...
	// * Charger les valeurs d'un champ $field ds un combobox conditioné par une valeur $valwhere du champ $wherefield et rediriger automatiquement vers la page $page_redirect.... A utiliser avec le menu de reroutage de dreamweaver.***
...
// * Charger les valeurs de deux champ $field1 et $field ds un combobox avec concaténation ex: prenom nom pour Guy ZINGUI***
	...
	// * Charger les valeurs de deux champ $field1 et $field ds un combobox avec un where clause avec concaténation ex: prenom nom pour Guy ZINGUI WHERE guy zingui = single usr***
...
// * Supprimer une valeur d'un champ -> d'un enregistrement ds un combo***
	function del_combo_entry($table, $field, $selected){
		...
		$result = mysql_query($query,$LibMan_Connect) or die("Impossible de supprimer...<br>".mysql_error());
		if($result){
		...
		return (mysql_affected_rows($LibMan_Connect));
	}
...
//* Sélectionner ts les enregistrements de la table $table indexés par le champ $field
	function combo_get_row_indexed($table,$field,$index){
		...
		if($total = mysql_num_rows($result, $LibMan_Connect)){
...
function get_id_tbl($tble,$field_id,$field_test,$usr_entry){ //ex: get_id_tbl("gpe","id_gpe","lib_gpe",$select_gpe) prend l'id_pe correspondant au lib_gpe choisi ds la table gpe
	...
	$result = mysql_query($query,$LibMan_Connect) or die("Erreur d'extraction <br>".mysql_error());
		if($total = mysql_num_rows($result)){
			while($row = mysql_fetch_object($result)){
...
function adduser_bsa($login,$pass,$matric,$gpe_id){
	global $LibMan_Connect;
	$result = mysql_query("INSERT INTO usr VALUES('$login', '$pass', '$matric', '$gpe_id')", $LibMan_Connect);
	return true;
}
...
function clear_tbl($tbl){
	...
	$result = mysql_query($query, $LibMan_Connect) or die("Erreur de DML<br>".mysql_error());
	if($result){
...
function query_interv($tbl, $date_fld, $val_date, $x){
	...
	if($result = mysql_query($query, $LibMan_Connect))
		return true;
	else 
...
function h_db_rate($tbl, $fld_id, $fld_value){
	...
	$result = mysql_query($query, $LibMan_Connect) or die("Erreur!! <br />".mysql_error());
	if($total = mysql_num_rows($result)){
		print "<table><tr>";
		while($row = mysql_fetch_object($result)){
...
function soltech_maj($page_id){
	...
	$result = mysql_query($query, $LibMan_Connect) or die("Impossible d'afficher les données<br />".mysql_error());
	if($total = mysql_num_rows($result)){
		while($row = mysql_fetch_object($result)){
...
//MODULE POUR L'AFFICHAGE RANDOMISE DES PRODUITS PHARES DE SOLUTION TECHNOLOGY
          //On teste si le nombre de produits phares est inferieur ou égal au maximum autorisé
...
function del_entry($tbl, $field, $where){
	...
	if($request = mysql_query($query, $LibMan_Connect))
		return true;
	else{
