...
//**************************************
	...
	$MyFileConnect = mysql_pconnect($host,$usr,$pass) or die("Erreur de connexion à la base de donn&eacute;es...<br>".mysql_error());
...
function chk_entry($tbl, $field, $entry)  //$tbl = latable a indexer, $field = le champ concerné, $entry = la valeur à tester prise ds le formulaire
	...
	$result = mysql_query($query, $MyFileConnect) or die ("Erreur d'extraction des données dans la base ".mysql_error());
	$row = mysql_fetch_row($result);
	if($total = $row[0])  //Mysql à généré au moins 1 resultat 
		return true;
	}
	
function chk_2entries($tbl, $field1, $field2, $entry1, $entry2)  //$tbl = latable a indexer, $field = le champ concerné, $entry = la valeur à tester prise ds le formulaire
	{
	global $MyFileConnect;
	$query = "SELECT * FROM `$tbl` WHERE `$field1`= '$entry1' AND `$field2`= '$entry2'";
	$result = mysql_query($query, $MyFileConnect) or die ("Erreur d'extr= '$entry'action des données dans la base ".mysql_error());
	$row = mysql_fetch_row($result);
	if($total = $row[0])  //Mysql à généré au moins 1 resultat 
		return true;
	}
...
function chk_entry_1($tbl, $field, $entry, $fieldtest, $entrytest)  //$tbl = latable a indexer, $field = le champ concerné, $val est la valeur fixe à comparer, $entry = la valeur à tester prise ds le formulaire
	...
	$result = mysql_query($query, $MyFileConnect) or die ("Erreur d'extraction des données de la base ".mysql_error());
	$row = mysql_fetch_row($result);
	if($total = $row[0])  //Mysql à généré au moins 1 resultat 
	return true;
	}
...
//*********** Fin d'ajout d'utilisateur ****************
	...
	function set_connected($table, $field, $login_field, $user_entry, $val){
		...
		$result = mysql_query($query,$MyFileConnect) or die("Erreur de mise à jour de champ <br>".mysql_error());
	}
...
// * Extraire noms et prénoms à partir de l'ID
...
function soltech_maj($page_id){
	...
	$result = mysql_query($query, $MyFileConnect) or die("Impossible d'afficher les données<br />".mysql_error());
	if($total = mysql_num_rows($result)){
		while($row = mysql_fetch_object($result)){
...
//
	...
	/*$table = la table de la bdd
	  $field = champ de la bdd concerné
	  $val = la valeur à inserer ds les options, ki seront récupérées
	  $chmp = le nom du champ à verifier*/
		global $MyFileConnect;
		$query = "SELECT $field_id, $field_val FROM $table ORDER BY $field_order";
		$result = mysql_query($query, $MyFileConnect) or die("Erreur de s&eacute;lection de donn&eacute;es ds la bdd <br>".mysql_error());
		if($total = mysql_num_rows($result)){
